...
### Cycle 7 — End-to-end (XCUITest) — TWO assertions

The user explicitly asked for both: **server-side proof** (RPC query) AND **UI proof** (the GPS visible in the Attendances list/form when the user opens it). Cycle 7 covers both.

**Failing test (`E2E_LocationTests.testE2E15_clockInRecordsAndDisplaysGPS`):**

```swift
func testE2E15_clockInRecordsAndDisplaysGPS() async throws {
    // === Setup (idempotent) ===
    // simctl privacy grant — done once in XCUITestPlan pre-actions:
    //   xcrun simctl privacy <udid> grant location io.woowtech.odoo.debug
    let app = XCUIApplication()
    app.launchArguments += ["-LocationEnabled", "-ResetAppState"]
    app.launch()
    try await loginIfNeeded(user: "admin", pass: "admin",
                            url: "https://<tunnel>", db: "odoo18_ecpay")

    // Snapshot Odoo state BEFORE clock-in
    let beforeId = try await OdooHelper.latestAttendanceID(forUserId: 2)

    // === Action: clock in via the WKWebView ===
    // Selectors derived from Odoo 18 OWL XML (same as Android E2E-15):
    //   addons/hr_attendance/static/src/components/attendance_menu/attendance_menu.xml
    // Approach: WKScriptMessageHandler debug-only "echo" lets us run JS from
    // the test process via a public test interface (DEBUG builds only).
    try await JSBridge.runInWebView(app, """
        document.querySelector('header i.fa-circle[aria-label="Attendance"]')
                .closest('button').dispatchMouseEventReal();
    """)
    try await Task.sleep(nanoseconds: 2_000_000_000)
    try await JSBridge.runInWebView(app, """
        document.querySelector('button.btn-warning > i.fa-sign-out, button.btn-success > i.fa-sign-in')
                .closest('button').dispatchMouseEventReal();
    """)
    try await Task.sleep(nanoseconds: 8_000_000_000)  // GPS + RPC

    // === Assertion 1: server-side via JSON-RPC ===
    let afterRecord = try await OdooHelper.latestAttendance(forUserId: 2)
    XCTAssertNotEqual(afterRecord.id, beforeId, "Expected a NEW or UPDATED hr.attendance row")
    let lat = afterRecord.in_latitude != 0 ? afterRecord.in_latitude : afterRecord.out_latitude
    let lon = afterRecord.in_longitude != 0 ? afterRecord.in_longitude : afterRecord.out_longitude
    XCTAssertNotEqual(lat, 0.0, "Server-side: expected non-zero lat on hr.attendance")
    XCTAssertNotEqual(lon, 0.0, "Server-side: expected non-zero lon on hr.attendance")

    // === Assertion 2: navigate to list view and SEE the GPS ===
    // User-facing proof — open /odoo/attendances list, click latest row,
    // form view shows in_latitude / in_longitude as text.
    try await JSBridge.runInWebView(app, "window.location.href = '/odoo/attendances';")
    try await Task.sleep(nanoseconds: 3_000_000_000)

    // Click the first row in the list to open the form view
    try await JSBridge.runInWebView(app, """
        document.querySelector('.o_list_view tbody tr.o_data_row')
                .dispatchMouseEventReal();
    """)
    try await Task.sleep(nanoseconds: 3_000_000_000)

    // Read the displayed lat/lon from the form view
    let displayed = try await JSBridge.runInWebView(app, """
        JSON.stringify({
            in_lat:  document.querySelector('input[name="in_latitude"], span[name="in_latitude"]')?.value
                  ?? document.querySelector('span[name="in_latitude"]')?.textContent.trim(),
            in_lng:  document.querySelector('input[name="in_longitude"], span[name="in_longitude"]')?.value
                  ?? document.querySelector('span[name="in_longitude"]')?.textContent.trim(),
            out_lat: document.querySelector('input[name="out_latitude"], span[name="out_latitude"]')?.value
                  ?? document.querySelector('span[name="out_latitude"]')?.textContent.trim(),
            out_lng: document.querySelector('input[name="out_longitude"], span[name="out_longitude"]')?.value
                  ?? document.querySelector('span[name="out_longitude"]')?.textContent.trim()
        })
    """)
    let parsed = try JSONDecoder().decode(DisplayedCoords.self, from: Data(displayed.utf8))
    XCTAssertTrue(
        (parsed.in_lat?.contains(".") == true) || (parsed.out_lat?.contains(".") == true),
        "UI: expected the form view to display non-zero latitude string for the latest attendance"
    )

    // === Cleanup (test-independence per CLAUDE.md) ===
    app.launchArguments += ["-LocationEnabled=false"]
    // (Or just let the next test's setUp own its baseline.)
}
```

**Why two assertions:** server-side answers "did the data land?", UI answers "would a real user see it?". They can diverge — e.g. RPC writes succeed but a Compose/OWL display bug renders 0.0 to the user. Catching both protects against the same class of bug we hit on Android (cosmetic feature with backing data missing).

**`JSBridge`** is a test-only helper that wraps a debug-only `WKScriptMessageHandler` named `__woowTestEval`. Public API:
```swift
@discardableResult
static func runInWebView(_ app: XCUIApplication, _ js: String) async throws -> String
```
The handler responds to `{"id": uuid, "code": "<js>"}` with `{"id": uuid, "result": "<stringified>"}` via `evaluateJavaScript` posting back to a dedicated test-receive function. The handler MUST be installed only in DEBUG builds (R8 / strip equivalent for iOS = `#if DEBUG`).

**`dispatchMouseEventReal`** is a small JS helper appended to the shim file — wraps `dispatchEvent` with `MouseEvent('mousedown')` + `MouseEvent('mouseup')` since OWL Dropdown ignores synthetic `.click()` (same lesson learned on Android with CDP `Input.dispatchMouseEvent`).

**Code:** wire the gate + coordinator + shim into `OdooWebViewCoordinator` configuration block. Add `JSBridge` test helper. Add `OdooHelper` JSON-RPC helper (mirrors the Python `odoo_execute` in Android `e2e_15_clockin_full.py`).
...
10. Real-device E2E: cold install → login → tap Sign In → permission prompt → grant → `hr.attendance.in_latitude` ≠ 0 (**server-side via JSON-RPC**)
11. Real-device E2E: navigate to `/odoo/attendances` list → tap first row → form view displays the same non-zero lat/lon (**UI-visible proof — user can see it**)
12. Real-device negative: toggle off → next clock-in records 0.0 (Odoo fallback)
13. Real-device permanent deny path → app surfaces Settings deep-link CTA via snackbar (NOT re-prompt — iOS doesn't allow that)
